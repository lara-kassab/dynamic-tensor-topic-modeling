...
class Online_NTF:
    def __init__(
        ...
        Seeks to find nonnegative tensor factorization
            X \\approx \\sum W^l * H^l * T^L

            W.shape = (d, r), H.shape = (n, r), T.shape = (m, r)
        n_components (int):
            r = number of columns in dictionary matrix W
                where each column represents on topic/feature
        iter (int):
            number of iterations where each iteration is a call to step(...)
        batch_size (int):
            number random of columns of X to be sampled during each iteration
        """
        self.X = X
        ...
        self.alpha = alpha
        # mode of unfolding the input tensor to learn marginal dictioanry by
        # OMF problem
        self.mode = mode
    ...
    def joint_sparse_code_tensor(self, X, W):
        ...
        args:
            X (ndarray):
                data matrix with dimensions: features (d) x samples (n)
            W (ndarray):
                dictionary matrix with dimensions: features (d) x topics (r)

        returns:
            H (ndarray):
                code matrix with dimensions: topics (r) x samples(n)
        """
    ...
    def update_dict(self, W, A, B):
        ...
        args:
            W (ndarray):
                dictionary matrix with dimensions: data_dim (d) x topics (r)
            A (ndarray):
                aggregate matrix with dimensions: topics (r) x topics(r)
            B (ndarray):
                aggregate matrix with dimensions: topics (r) x data_dim (d)

          returns:
            W1 (ndarray):
                updated dictionary matrix with dimensions:
                    features (d) x topics (r)
        """
    ...
    def step(self, X, A, B, W, t):
        ...
        Han's Markov paper.
        Note: H (ndarray): code matrix with dimensions:
            topics (r) x samples(n)

        args:
            X (ndarray): data matrix with dimensions:
                data_dim (d) x samples (n)
            A (ndarray): aggregate matrix with dimensions:
                topics (r) x topics(r)
            B (ndarray): aggregate matrix with dimensions:
                topics (r) x data_dim(d)
            W (ndarray): dictionary matrix with dimensions:
                data_dim (d) x topics (r)
            t (int): current iteration of the online algorithm

        returns:
            Updated versions of H, A, B, and W after one iteration of
            the online NMF (H1, A1, B1, and W1 respectively)
        """
    ...
    def train_dict_single(self):
        """
        Given data tensor X and mode=i, learn dictionary matrix W and the
        complementary joint sparse code H.
        Reduce to matrix factorization by unfolding X along mode i

        ---------------if 'learn_joint_dict' = False:
        args:
            X (ndarray): data tensor with dimensions = (d) x (n) x (m)
            W (ndarray): dictionary matrix with dimensions
            =  features (d) x topics (r) if mode=0
            =  features (n) x topics (r) if mode=1
            =  features (m) x topics (r) if mode=2

        method:
            X(i) = mode-i (Katri-Rao) unfolding of tensor X with dimensions
            = (d) x (n m) if mode = 0
            = (n) x (d m) if mode = 1
            = (m) x (d n) if mode = 2
            find sparse code H such that X(i) approx W @ H using online matrix
            factorization

        returns:
            H (ndarray): code matrix with dimensions
            = (r) x (n m) if mode = 0
            = (r) x (d m) if mode = 1
            = (r) x (d n) if mode = 2
        ...
        args:
            X (ndarray): data tensor with dimensions = (d) x (n) x (m)
            W (ndarray): dictionary matrix with dimensions
            =  features (n m) x topics (r) if mode=0
        ...
        method:
            X(i) = mode-i (Katri-Rao) unfolding of tensor X with dimensions
                ...
                = (d m) x (n) if mode = 1
                = (d n) x (m) if mode = 2
            find sparse code H such that X(i) approximately W @ H using online
