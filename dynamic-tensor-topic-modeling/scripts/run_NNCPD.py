...
Args:
    ...
    local_path (str): directory containing the tensor and features data (a pickle file).
    ...
    factors_path (str): relative file path in local_path to save/load the NNCPD factors
    weights_path (str): relative file path in local_path to save/load the NNCPD weights
"""
...
local_path = results_dir  # Fill
tensor_filename = "tweet_tensor.pickle"  # Fill
...
# Load data tensor
directory = os.path.join(data_dir, tensor_filename)
...
for i, topic in enumerate(factors[1].T):
    ...
    print(
        "Topic {}: {}".format(
            i,
            ", ".join(x for x in reversed(np.array(features)[topic.argsort()[-10:]])),
        )
    )
